...
| Frontend | React + Vite + Tailwind (`Parcial1_Sw2_Frontend/`, repo separado de `hebertsb`) | Login con Google conectado y probado end-to-end contra Supabase real, + agente de voz (`VoiceAgent.tsx`) ya conectado al servicio de IA real (`src/api/voice.api.ts`, ver abajo). Panel admin con datos reales (no mockup): cartelera/funciones, salas, usuarios, reportes, auditoría, y **Precios & Promos** (CRUD completo, agregado 2026-09-15 — antes era un placeholder "Módulo en Desarrollo"). Pendiente conocido: dulcería sigue con datos hardcodeados en el carrito (`CandyBarSelection.tsx`), `/admin/console` no tiene guard de rol, y el checkbox de no-reembolso (RF03) está fijo en `true` sin captura real del usuario. |
| App móvil cliente | Flutter (`mobile-app/`) | No existe todavía |
| Servicio de IA (voz/NLU) | FastAPI, repo propio `Backend_IA/agent_cine` (a cargo de Elías + el dev del frontend) | Funcionando de punta a punta. El frontend le habla por WebSocket (`/ws/voz`, conversación continua con interrupción) y por HTTP (`POST /voice-chat`), fuera del NestJS. Las mutaciones reales llegan a la base **solo** por `ia-gateway`, tras la confirmación verbal (RF10/RF19): `gateway_client.py` ejecuta la gestión de admin y la compra del cliente (`crear_venta` + `crear_pago`). **Límite conocido**: `ia-gateway` es `@Public()` y confía en el `idUsuario` y el `rol` que manda el servicio de IA; falta autenticación servicio a servicio y verificar el JWT. Detalle en `Backend_IA/agent_cine/ESTADO-IMPLEMENTACION.md`. |
