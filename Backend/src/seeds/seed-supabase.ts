...
/**
 * Seed script for Supabase (production) - runs in smaller batches
 */
...
const toDateKey = (d: string | Date): string => d instanceof Date ? d.toISOString().split('T')[0] : d.split('T')[0];

async function seedSupabase() {
  console.log('🌱 Iniciando seed en Supabase (batch mode)...');
  ...
  try {
    // 1. Funciones adicionales (batch de 50)
    console.log('\n🎬 1. Creando funciones adicionales...');
    ...
    for (let d = new Date(fechaBase); d <= fechaFin; d.setDate(d.getDate() + 1)) {
      ...
      if (funcCount % 50 === 0 && funcCount > 0) {
        await qr.commitTransaction(); console.log(`  ${funcCount} funciones...`); await qr.startTransaction();
      }
    }
    console.log(`   ✅ ${funcCount} funciones nuevas creadas`);

    // 2. Promociones a funciones
    console.log('\n🎫 2. Asignando promociones...');
    ...
    // 3. Ventas masivas (en lotes de 100)
    console.log('\n💰 3. Generando ventas (~1000 en lotes)...');
    ...
    for (let batch = 0; batch < 10; batch++) { // 10 lotes = 1000 ventas
      for (let i = 0; i < LOTE; i++) {
        const fn = pick(ventaFuncs);
        const asientos = (asientosPorSala.get(fn.idSala) || []).filter(a => {
          const d = qr.manager.findOne(DisponibilidadAsiento, { where: { idFuncion: fn.idFuncion, idAsiento: a.idAsiento } });
          return true; // simplified
        });
        if (asientos.length === 0) continue;
        const numEnt = weightedPick([1, 2, 3, 4], [50, 30, 15, 5]);
        const asientosSel = shuffle(asientos).slice(0, numEnt);
      ...
      await qr.commitTransaction(); console.log(`  ${totalVentas} ventas...`); await qr.startTransaction();
    }
    ...
    console.log('\n📊 Stats:', { ventas: stats[0], pagos: stats[1], detalle_entradas: stats[2], detalle_dulceria: stats[3], funciones: stats[4], promo_func: stats[5], logs: stats[6] });
    
  } catch (e) { await qr.rollbackTransaction(); console.error('❌ Error:', e); throw e; }
